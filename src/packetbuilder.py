...
def form_phy_payload(
    ...
    encryptor = AES(
        unhexlify(reverse_hex_order(devaddr, verbose)),
        unhexlify(appskey),
        unhexlify(nwkskey),
        fcnt,
    )
...
def form_udp_message(
    ...
    verbose=False,
    tmst=3003866827,
    time="2022-08-10T10:00:17.256635Z",
    chan=1,
    rfch=1,
    freq=868.300000,
    stat=1,
    ...
    if verbose:
        print(f"UDP PUSH-DATA JSON = {json.dumps(json_dict, separators=(',', ':'))}")
    json_obj = string_to_hex_string(json.dumps(json_dict, separators=(",", ":")))
    protocol_vers_hex = unhexlify(protocol_version).hex()
    rand_tok_hex = unhexlify(rand_tok).hex()
    push_data_hex = unhexlify("00").hex()
    message = f"{protocol_vers_hex}{rand_tok_hex}{push_data_hex}{gateway_eui}{json_obj}"
    if verbose:
